...
XML;
	...
	/** @param \Throwable $throwable 예외를 xml로 출력 */
	public function publish(\Throwable $throwable) {
		...
		$traceExceptions = $throwable->getTrace();
		...
		$exceptionTraceStr = sprintf(
			...
			$this->createPreviousString($throwable->getPrevious())
		);
		$response          = sprintf(self::XML_WRAPPER, $exceptionTraceStr, $callStacksStrBuilder->toString());
		echo $response;

		$this->writeResponse($throwable, $response);
		die(-1);
	}
	...
	private function writeResponse(\Throwable $throwable, $content) {
		file_put_contents($this->createExceptionLogFileName($throwable), $content);
	}
	 ...
	 * @return string
	 */
	private function createExceptionLogFileName(\Throwable $throwable) {
		$dir = _DIR_LOG_PHP_EXCEPTION_ . date('Ymd') . DIRECTORY_SEPARATOR;
		if (!file_exists($dir))
			mkdir($dir, 7666, true);

		return $dir . date('His') . '-' . trim(str_replace("\\", '.', get_class($throwable)), '.') . 'response.xml';
	}
}