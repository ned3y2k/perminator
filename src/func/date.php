...
function date_extract_set($start_date, $end_date) {
	return implode(',', date_explode($start_date, $end_date));
}
 ...
 * @param string $end_date
 *
 * @param int $returnType 0 은 string 1은 datetime
 *
 * @return string[]
 * @throws Exception
 */
function date_explode($start_date, $end_date, $returnType = 0) {
	$dateSetArray = array();
	...
	elseif ($returnType == 1)
		$dateSetArray[ ] = new DateTime($start_date);
	...
	if ($returnType == 0)
		...
		for ($i = 1; $i < $remain_day; $i++) {
			$dateSetArray[ ] = new DateTime(date_add_day($start_date, $i));
		}
	if ($returnType == 0)
		$dateSetArray[ ] = $end_date;
	elseif ($returnType == 1)
		$dateSetArray[ ] = new DateTime($end_date);
 ...
 * @throws Exception
 */
function date_time_explode($begin, $end) {
	$days        = array();
 ...
 *
 * @return array <string, number> hour, min, sec, day
 */
