 ...
 */
function print_alert_location_back($msg, $isExit = false, $charset = 'utf-8') {
	getApplicationContext()->getResponseContext()->setContextType('text/html', $charset);
...
function html_checkbox($checkboxName, $valueList, $nameList, array $selectedValues = null) {
	if ($selectedValues == null) $selectedValues = array();
...
function html_option_hour($hour) {
	...
	for ($i = 0; $i < 24; $i++) {
		...
		if ($hour == $i)
			...
			$temp[ ] = $i ? "<option value='{$str}'>{$i}</option>" : "<option value='{$str}'>{$i}</option>";
	}
