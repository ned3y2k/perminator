 ...
 * @param string  $destinationDir     업로드할 디렉토리
 ...
 * @return NULL|string
 */
function file_upload_process($key, $destinationDir, $newName = null, $returnFullPath = false, $newNameKeepExtName = true) {
	...
	if(!move_uploaded_file ( $_FILES [$key] ['tmp_name'], $destinationDir.$newName )) throw new RuntimeException('file upload fail');

	return $returnFullPath ? $destinationDir . $newName : $newName;
}
